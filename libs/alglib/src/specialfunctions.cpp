...
double invincompletebeta(double a, double b, double y, ae_state *_state)
{
     ...
     */
/*
     * 이러한 초기화는 실제로 필요하지 않습니다.
     * 컴파일러가 없으면 컴파일러는 '초기화되지 않은 변수'에 대해 불평합니다.
*/
...
/*
불완전한 베타 적분을위한 멱급수.
b * x가 작고 x가 1에 너무 가깝지 않을 때 사용하십시오.
Cephes 수학 라이브러리, 릴리스 2.8 : 2000 년 6 월
Copyright 1984, 1995, 2000 Stephen L. Moshier
*/
     ...
     ae_state *_state)
{
    ...
    /*
     * Special cases: N=0 or N=1	 * 특수 사례 : N = 0 또는 N = 1
     */
    if( n==0 )
    {
        result = a;
...
/*
 * Brian Fitzgerald에게 감사드립니다. <fitzgb@mml0.meche.rpi.edu>
     * pi / 2의 홀수 배수 근처에서 불안정성을 지적하기 위해
*/
    ...
    if( ae_fp_greater(ae_fabs(t, _state),10) )
    {
        ...
        /*
         * Transform the amplitude	 * 진폭 변환
         */
        e = 1.0/(b*t);
        
        /*
         * ... but avoid multiple recursions.	  * ... 다중 재귀를 피하십시오.
         */
        if( ae_fp_less(ae_fabs(e, _state),10) )
        {
            e = ae_atan(e, _state);
     ...
     ae_state *_state)
{
    ...
    /*
     * Compute probability for x = 0.5	 * x = 0.5에 대한 확률 계산
     */	 
...
/*
     * y보다 큰 경우 w <.5의 해
     * 그렇지 않으면 1-y에서 풀면 (b - b * w)에서 취소가 제거됩니다.
*/
